...
type Props = {
  ...
  onChange: (entries: TrainingEntry[]) => void;
};
  ...
  return (
    <div className="flex flex-col gap-2">
      {entries.map((entry, i) => (
        <div
          key={i}
          className="flex items-center gap-2 rounded-[12px] bg-[#F5F5F5] p-3"
        >
          ...
          <Input
            ...
            className="w-16 text-center rounded-[8px]"
            min={1}
          />
          <span className="text-xs text-[#707072]">회 중</span>
          <Input
            type="number"
            value={entry.success}
            ...
            min={0}
          />
          <span className="text-xs text-[#707072]">성공</span>
          <button
            onClick={() => removeEntry(i)}
            className="text-[#9E9EA0] text-lg"
          >
            ×
          </button>
        </div>
      ...
      <div className="flex gap-2">
        ...
        />
        <Button onClick={addEntry} variant="outline" className="rounded-[16px]">
          추가
        </Button>
      </div>
    </div>
