  ...
  return (
    ...
    >
      ...
      <p className="mb-3 text-sm font-bold text-[#111111]">
        ...
        {" · "}
        {MEALS_LABELS[report.meals_eaten as keyof typeof MEALS_LABELS] ??
          report.meals_eaten}
      </p>

      {report.ai_summary && (
        <p className="mb-3 text-sm text-[#111111] leading-relaxed">
          {report.ai_summary}
        </p>
      )}
      ...
      <div className="flex gap-4 text-xs text-[#707072]">
        <span>
          🚶 {report.walk_count}회 {report.walk_distance_km}km
        </span>
        {report.food_brand_today && <span>🍽 {report.food_brand_today}</span>}
      </div>

      {report.teacher_note && (
        <p className="mt-2 text-xs text-[#707072] italic">
          "{report.teacher_note}"
        </p>
      )}
